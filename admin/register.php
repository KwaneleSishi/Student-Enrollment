...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        // Fetch department ID dynamically
        $stmt = $conn->prepare("SELECT id FROM departments WHERE name = ?");
        $stmt->execute([$_POST['major']]);
...
$departments = $conn->query("SELECT id, name FROM departments")->fetchAll();
?>
                ...
                <div class="form-group">
                    ...
                    <select id="major" name="major" class="form-control" required>
                        ...
                        <?php foreach ($departments as $dept): ?>
                            <option value="<?= htmlspecialchars($dept['name']) ?>" <?= $preservedValues['major'] === $dept['name'] ? 'selected' : '' ?>>
